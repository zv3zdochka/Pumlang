...
class Lexer:
    ...
    def make_lexemes(code):
        ...
        if tokens.count('{') != tokens.count('}') or tokens.count('(') != tokens.count(')'):
            print(colorama.Fore.RED + 'Program must have an equal number of opening and closing brackets of all types')
            exit(-1)
        elif tokens[0] == '{' and tokens[len(tokens) - 1] == '}':
            return tokens

        else:
            print(colorama.Fore.RED + 'Program must be in curly brackets: {your code}')
            exit(-1)
    ...
    def parse(self):
        ...
        class Parser:
            ...
            def error(self, description):
                print(
                    colorama.Fore.RED + f"Invalid syntax error at line {self.line_number}: \033[4m{' ' + self.current + ' '}\033[0m." + "\n" + colorama.Fore.RED + f"{description}")
                exit(-1)
            ...
            def P(self):
                self.state = "B"
                self.B()
                print(colorama.Fore.GREEN + 'Program finished')
                return
            ...
            def S(self):
                ...
                if self.current in ['{', '}']:
                    ...
                    if self.current in self.mem:
                        self.error(
                            f'Variable with the same name have already used before')
        ...
        prn = Prn(self.text)
        print(self.text)
        pol_not = prn.infix_to_postfix()
        print(pol_not)

with open('main.plg', 'r', encoding='UTF-8') as m_file:
    inp_t = m_file.read()
    L = Lexer(inp_t)
    L.parse()
