...
### 1. Перевод в постфиксную запись
 ...
 Перевод в польскую запись осуществляется в файле Polish_reversed_notation. Который запускается из файла pumlang.py. 
 По итогу после перевода в постфиксную запись (она же польская) мы получаем вот такой список элементоов, уже готовый к исполнению.
...
### 3. Внутреннее исполнение программы
