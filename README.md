...
Как уже умомяналось ранее, при написании кода не использовались какие-либо специальные библиотеки, кроме math и sys, да и то для того чтобы запускать все через командную строку.
...
Изначально планируется что код написанный на языке Pumlang будет запускаться через командную строку. Файл в котором содержится программа написанная на языке Pumlang называется main.plg. Основной файл в проекте на которой м ыи будем ссылаться в командной строке называется pumlang.py. 
