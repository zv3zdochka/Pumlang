...
1. P (Programm) -> B&
   - Программа состоит из блока инструкций B, за которым может следовать символ "&", работающий как exit() в python.
...
При написании кода не использовались какие-либо специальные библиотеки, кроме sys, да и то для того чтобы запускать все через командную строку.
...
5
```
Это было сложно но оно работает попробуем что нибудь потяжелее. 
```python
С:\Users\PumLang> python pumlang.py main.plg
>>> 1000000
#five years later
...
И вот мы дописали наш интерпретаатор на питоне. Да, пусть он и не обладает какими-то преимуществами относительно питона из за плохой оптимизации и в целом самой идеи написание интерпретатора на питоне. Но в ходе написания этого интерпретатора были подробно разложены по полочкам все аспекты написания вместе с теорией и фрагментами кода, поэтому как образовательный кейс данный проект отлично подойдет. Если у вас возникнут какие либо вопросы можете писать на почту mai.batsiev.oleg@gmail.com 
